...
public class DetailItemReader {
    ...
    public List<DetailItem> read(InputStream inputStream) throws JSONException {

        ArrayList<DetailItem> item = new ArrayList<>();
        String json = new Scanner(inputStream).useDelimiter(REGEX_INPUT_BOUNDARY_BEGINNING).next();
        JSONArray array = new JSONArray(json);
        String District;
        int CountyNum;
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            item.add(new DetailItem(title, ssnippet, id, District,
                    CountyNum,
                    COUNTY,
                    NameMobileWeb,
                    LocationMobileWeb, DescriptionMobileWeb, PHONE_NMBR, LIST_ORDER, GEOGR_AREA, LATITUDE, LONGITUDE,
